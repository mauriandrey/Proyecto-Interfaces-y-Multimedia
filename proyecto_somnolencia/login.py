...
class LoginWindow:
    def __init__(self):
        ...
        # Cargar logo
        try:
            imagen_original = Image.open("Logo.png")  # Asegúrate de que la ruta sea correcta
    ...
    def ingresar(self):
        """
        Se llama a la función 'ingreso_usuario' (definida en registro.py) para realizar el login.
        Si es exitoso, se cierra la ventana de login y se inicia la pantalla de carga.
        Se asume que 'ingreso_usuario' retorna True en caso de login correcto.
        """
        # Suponemos que ingreso_usuario() retorna True si el login es correcto.
        if ingreso_usuario():
            messagebox.showinfo("Ingreso Exitoso", "Has ingresado correctamente.")
            self.root.destroy()  # Cierra la ventana de login
            show_loading_screen()  # Inicia la pantalla de carga y, posteriormente, la interfaz principal
        else:
