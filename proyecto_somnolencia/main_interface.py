...
class MainInterface:
    def __init__(self, root=None):
        ...
        self.bienvenida = tk.Label(self.frame_superior, text="¡Bienvenido!, siempre es un gusto tener aquí <usuario>",
        ...
        # Botón: Actividades
        self.btn_actividades = tk.Button(self.frame_menu, text="Actividades",
                                         command=self.open_activities, **btn_style)
        self.btn_actividades.pack(pady=10, fill=tk.X)

        # Botón: Test de Somnolencia
        self.btn_test = tk.Button(self.frame_menu, text="Test de Somnolencia",
                                  command=self.open_sleep_test, **btn_style)
        self.btn_test.pack(pady=10, fill=tk.X)

        # Botón: Resultados
        self.btn_resultados = tk.Button(self.frame_menu, text="Resultados",
                                        command=self.open_results, **btn_style)
        self.btn_resultados.pack(pady=10, fill=tk.X)

        # Botón: Salir (con mayor margen para separarlo)
        self.btn_salir = tk.Button(self.frame_menu, text="Salir", command=self.root.quit, **btn_style)
        self.btn_salir.pack(pady=100, fill=tk.X)
        ...
        # Inicializar el monitor de somnolencia
    ...
    # Funciones para abrir las ventanas de las funcionalidades
    ...
    def toggle_camera(self):
        if self.running:
            # Apagar la cámara
            self.running = False
            self.btn_camara.config(text="Iniciar cámara", bg="#4caf50")
            ...
            if not self.cap.isOpened():
                ...
                return
            self.running = True
            self.btn_camara.config(text="Apagar cámara", bg="#d32f2f")
        ...
        if ret:
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    # Actualiza el análisis de somnolencia y dibuja sobre el frame
            ...
            # Convertir nuevamente a RGB para mostrar
